...
LEFT JOIN tRoles r ON u.IdRol = r.IdRol");
    ...
    {
        try {
            $stmt = $this->db->prepare("SELECT p.CodPermiso, p.CodMenu, p.IdRol AS IdRol,
                                       p.Permiso, m.NombreMenu, m.MenuRuta, m.MenuIdentificador, 
                                       m.MenuIcono, m.MenuGrupo, m.MenuGrupoIcono,
                                       m.Estado as EstadoMenu, r.NombreRol 
                                       FROM tpermisos p
